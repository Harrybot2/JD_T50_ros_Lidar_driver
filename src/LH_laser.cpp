...
{
  LHLaser::LHLaser(const std::string& port, uint32_t baud_rate, uint32_t firmware, boost::asio::io_service& io): port_(port),
...
int K=0;
  void LHLaser::poll(sensor_msgs::LaserScan::Ptr scan) {
   ...
   if(firmware_ == 2) { 
      ...
      rpms=0;
      int index=0;
      printf("0**shutting_down_:%d;got_scan is:%d\n",shutting_down_,got_scan);
      while (!shutting_down_ && !got_scan) 
      {
	boost::asio::read(serial_, boost::asio::buffer(&raw_bytes[start_count],1));
	printf("01**start_count:%d;raw_bytes[start_count] is:%02x\n",start_count,raw_bytes[start_count]);
	if(start_count == 0)
	{
	...
	else if(start_count == 1)
	{
	 ...
	 {
	    printf("02**start_count:%d;raw_bytes[start_count] is:%x\n",start_count,raw_bytes[start_count]);
	    start_count = 0;
	    ...
	    printf("1***j_angle is:%d;j_num_readings point is:%d;CheckSum is:%d\n",j_angle,j_num_readings,CheckSum);
      ...
      printf("2***j_angle is:%d;j_num_readings point is:%d;CheckSum is:%d\n",j_angle,j_num_readings,CheckSum);
      if((uint16_t)CheckSum==(uint16_t)(raw_bytes[6+j_num_readings*2] + (raw_bytes[7 +j_num_readings*2])*256u))
        printf("3...j_angle=%d 's reading is ok\n",j_angle);
      else{
           printf("4...j_angle%d 's CheckSum is bad\n",j_angle);
           continue;
          }
      if(j_angle==3240)
      {
        ...
        for(int i=0;i<10;i++)
	{
	   ...
	   {
	     j_s_num_readings=0;continue;
           }
      j_s_num_readings=j_s_num_readings+A[i][0];
	}
      printf("5...the cloud point size for the beginning%d\n",j_s_num_readings);
	...
	scan->intensities.resize(j_s_num_readings);
        printf("6...the cloud point size%d\n",j_s_num_readings);
        j_size=0;
        for(int j=0;j<10;j++)
